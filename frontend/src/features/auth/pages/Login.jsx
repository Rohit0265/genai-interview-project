...
const Login = () => {
  ...
  if(loading){
    return <Loading variant="full-screen" message="Signing in to PrepAI..." description="Please wait while we verify your credentials." />
  }
  ...
  return (
    <main className="min-h-screen w-full flex flex-col justify-between bg-[#f1f5f9] dark:bg-[#0b0f19] text-slate-800 dark:text-slate-200 p-6 font-sans">
      ...
      <div className="w-full max-w-7xl mx-auto flex justify-between items-center mb-6">
        <span className="text-xl font-bold text-slate-900 dark:text-white tracking-tight">PrepAI</span>
        
        {/* Theme Toggle Button */}
        <button
          type="button"
          onClick={toggleTheme}
          className="p-2 text-slate-505 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-100 rounded-full hover:bg-slate-200/50 dark:hover:bg-slate-850 transition-all cursor-pointer"
      ...
      <div className="w-full flex-1 flex items-center justify-center py-6">
        <div className="w-full max-w-[420px] bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-8 shadow-lg dark:shadow-none">
          <h1 className="text-2xl font-bold text-center text-slate-900 dark:text-white mb-1">Welcome back</h1>
          <p className="text-xs text-slate-505 dark:text-slate-400 text-center mb-6">
          ...
          <p className="text-xs text-slate-600 dark:text-slate-400 text-center mt-6">
            ...
            <Link to="/register" className="text-[#0652dd] dark:text-blue-400 font-bold hover:underline">
              Create an account
            </Link>
          </p>
        </div>
      ...
      <footer className="w-full max-w-7xl mx-auto border-t border-slate-200/80 dark:border-slate-800 pt-6 mt-6 flex flex-col md:flex-row md:justify-between items-center space-y-4 md:space-y-0">
        <div className="flex flex-col md:flex-row items-center md:space-x-8 space-y-2 md:space-y-0">
          <span className="text-sm font-bold text-slate-900 dark:text-white">PrepAI</span>
        ...
        <div className="text-[10px] sm:text-xs text-slate-400 dark:text-slate-550 text-center md:text-right">
          © 2026 PrepAI. All rights reserved. Professional Coaching Platform.
        </div>
      </footer>
    </main>
  )
